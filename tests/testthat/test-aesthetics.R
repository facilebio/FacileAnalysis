...
test_that("create_color_map maps RColorBrewer color palettes to levels", {
  ...
  for (pal in pal.names) {
    ...
    expect_color_map(map, vals, info = paste("Small n,", pal))
    expect_equal(unname(map), expected)
  }
})
...
test_that("create_color_map recycle colors when necessary", {
  ...
  expect_color_map(map, vals, info = "more elements than colors")
...
test_that("aes maps can be set individually", {
  ...
  amap <- aes_map(x)
  expect_color_map(amap$color, x$category)
})
