...
# Install the airway package if you don't have it already
...
We will create that dataset, then subset out only the "airway," which will be
used as the basis for all "facile"-versions of the analyses, in our comparisons
against what is done in the rnaseqGene worfklow.
...
:::note
As outlined in the [FacileDataSet Assembly][fdsassembly] vignette, when these
...
highly loaded on the principla components, as well.
...
```{r fpcavsd, message=FALSE, warning=FALSE, eval=FALSE}
vpca <- fpca(vsd)
...
You can extract the differential statistics via the `tidy()` function.
...
```{r}
ggplot(cmp, aes(x = log2FoldChange, y = logFC.vm)) +
...
```{r}
cmp2 <- tidy(dex.qlf) %>% 
  select(feature_id, logFC.qlf = logFC, pval.qlf = pval) %>% 
  inner_join(cmp, by = "feature_id") %>% 
...
```{r pairs-pval, message = FALSE, warning = FALSE, fig.asp = 1, fig.width = 3.3}
cmp2 %>% 
  select(qlf.pval, vm.pval, d2.pval) %>% 
  pairs(main = "-log10(pvalue) comparison", pch = 16, col = "#3f3f3f33")
```

```{r pairs-logFC, message = FALSE, warning = FALSE, fig.asp = 1, fig.width = 3.3}
cmp2 %>% 
...
can use, then marrying the surrogate variables that were estimated back with
the FacileDataSet for use within the differential expression analysis workflow.

To extract the data from the FacileDataSet for `svaseq`, we will:
...
We can do a bit better, though. We can ask a `FacileAnalysisResult`, like 
`dex.qlf`, what samples it was run on using the `samples()` function:
...
2. The `lib.size` and `norm.factors` columns were added from the DGEList that
   ultimately materialized and used within the call to `fdge()`. These were
   recaculated "fresh" after we filtered down to the genes that were specified
...
ggplot(cmp.qlf, aes(x = logFC.x, y = logFC.y)) +
  ...
  xlab("logFC original") +
  ylab("logFC with sva") +
  ggtitle("logFC comparison with and without svaseq")
```
