...
The benefit of having the `fpca()` result (`pca`) in your hand is that you
can perform further action on it. `FacileAnalysisResult` objects, like `pca` 
in this case, provide a number of functions that enable their interrogation at
...
`FacileAnalysisResult` object provides its results in some tabular form via a
...
The `ranks(fpca())` produces a table of features that are highly loaded along
the PCs we calculated. `signature(fpca())` produces a summary of ranks, which
provides the `ntop` features positively and negatively loaded on each PC.
...
For a `FacilePcaAnalysisResult`, the `shine()` presents the user with shiny
gadget that providesa an interactive scatter plot of the samples over the
PC's of choice. The scatterplot can be modified to map the various sample-level
covariates to the aesthetics of the plot. Furthermore, the user is presented
with the features (genes) that are loaded most strongly along each principal
component.

```{r shine-pca, eval = FALSE}
shine(pca)
```
...
:::tip
In order to help users better understand what might be driving the variance
along each of the principal components, users can also perform gene set
enrichment analysis along a given PC. This provides the opportunity to see
if groups of genes are loaded together no the PC. An example of this is given
in the next section, after we remove batch effects from this data.
:::

#### Embeddable interactive graphics with viz()

We can embed a javascript-powered interactive scatter plot from the PCA plot
using the `viz` function. We use its `color_aes` and `shape_aes` to map the
same sample-level covariates to aeshtetics in the plot, as we did when we
explored this object via `shine()`, above.
